...
The build deliberately does not vendor another SDL or Python copy into the repository. Supply these paths through Gradle properties or the equivalent environment variables:
...
| `gameAndroidDependencyPrefix` | `GAME_ANDROID_DEPENDENCY_PREFIX` | Android dependency prefix containing CMake packages and ARM64 libraries for the repository's existing `pybind11`, SDL2, SDL2_image, and SDL2_ttf dependencies |
...
Official CPython Android distributions are supported by the layout above. The application copies the Python standard library into `files/python/lib/pythonX.Y` and packages `libpython` plus the CPython support `.so` libraries as JNI libraries.

## Dependency prefix with vcpkg

The repository already declares `pybind11`, `sdl2`, `sdl2-image`, and `sdl2-ttf` in `vcpkg.json`. An ARM64 Android vcpkg installation can therefore be used as `GAME_ANDROID_DEPENDENCY_PREFIX` when its triplet provides the matching Android libraries and CMake package files.
...
4. packages ARM64 `.so` dependencies from the Python and dependency prefixes;
