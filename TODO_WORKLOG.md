...
## Batch 21
- Location: `res/images/item.png`, `test.py`
- Original TODO or summary: A map actor on grass could render with opaque black matte islands because the generic item sprite still shipped with an opaque border palette instead of real transparency.
- Status: fixed
- What was changed: Audited the render path and confirmed the screenshot-class artifact was not coming from `CMapGraphicsObject`, `CAnimation`, or the footprint overlay. Then converted `res/images/item.png` so every RGB color present on the outer border becomes transparent everywhere in that PNG, and added a focused regression test that asserts the item sprite's border palette is transparent and no opaque matte pixels remain.
- Why the change is correct: `CTextureUtil::calculateAlpha(...)` only synthesizes transparency from the PNG data it receives. The verified offending sprite was `images/item`, used by the `preciousAmulet` `CItem` definition in `res/maps/nouraajd/config.json`; the raw asset had no alpha, all four corners were opaque `(2, 2, 0, 255)`, and `180666` pixels matching the border matte palette were still opaque. After the asset fix, the same PNG has transparent corners and zero opaque pixels using that border palette, so the artifact is removed without changing global masking behavior for unrelated sprites.
- Validation performed:
  - source verification of `AGENTS.md`, `README.md`, `docs/testing.md`, `src/gui/CTextureCache.cpp`, `src/gui/CTextureCache.h`, `src/gui/CAnimation.cpp`, `src/gui/CAnimation.h`, `src/gui/object/CMapGraphicsObject.cpp`, `src/gui/object/CMapGraphicsObject.h`, `src/gui/object/CGameGraphicsObject.cpp`, `src/gui/object/CGameGraphicsObject.h`, `res/config/graphics.json`, and `TODO_WORKLOG.md`
  - GitHub `main` verification of `src/gui/CTextureCache.cpp`, `src/gui/CTextureCache.h`, `src/gui/CAnimation.cpp`, `src/gui/object/CMapGraphicsObject.cpp`, `src/gui/object/CGameGraphicsObject.cpp`, `res/config/graphics.json`, and `TODO_WORKLOG.md` before editing
  - focused asset audit comparing `/home/andrz/git/fall-of-nouraajd/res/images/item.png` vs `/tmp/fon-black-render-artifacts/res/images/item.png` -> original corners were opaque and had `180666` opaque border-palette pixels; fixed asset corners are transparent and have `0` opaque border-palette pixels
  - `black -l 120 test.py` -> `1 file left unchanged`
  - `python3 -m unittest test.GameTest.test_item_sprite_border_palette_is_transparent` -> `Ran 1 test in 0.298s`, `OK`
  - `./configure.sh` -> configured `cmake-build-debug` and `cmake-build-release`; printed existing `sudo` and `externally-managed-environment` warnings but completed CMake generation successfully
  - `CCACHE_DIR=/tmp/fon-ccache cmake --build cmake-build-release --target _game for_unit_tests -j$(nproc)` -> built `_game` and `for_unit_tests` successfully after redirecting `ccache` to a writable directory
  - `ctest --test-dir cmake-build-release --output-on-failure -R for_unit_tests` -> `1/1 Test #1: for_unit_tests ... Passed`
  - `python3 test.py` -> `Ran 82 tests in 135.510s`, `OK`
  - `CCACHE_DIR=/tmp/fon-ccache ./scripts/run_coverage.sh` -> `Ran 82 tests in 234.993s`, `OK`, scoped line coverage `81.45% (2885/3542)`
- Blockers if unresolved: None. The existing `CMakeLists.txt` already packages `res/images/item.png`, so no resource-rule update was needed.
