...
- Validation performed:
  - `python3 -m py_compile res/plugins/object.py res/maps/nouraajd/script.py`
  - `cmake --build cmake-build-release --target _game for_unit_tests -j$(nproc)`
  - `ctest --test-dir cmake-build-release --output-on-failure -R for_unit_tests`
  - `python3 test.py` -> `Ran 41 tests in 104.655s`, `OK`
- Blockers if unresolved: `black --workers 1 -l 120 res/plugins/object.py res/maps/nouraajd/script.py` hangs in this environment instead of exiting cleanly.
