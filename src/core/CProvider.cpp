...
std::shared_ptr<json>
CConfigurationProvider::getConfig(const std::string &path) {
...
std::shared_ptr<json>
CConfigurationProvider::getConfiguration(const std::string &path) {
...
std::shared_ptr<CResourcesProvider> CResourcesProvider::getInstance() {
  static std::shared_ptr<CResourcesProvider> instance =
      std::make_shared<CResourcesProvider>();
  return instance;
}
...
CAnimationProvider::getAnimation(const std::shared_ptr<CGame> &game,
  ...
  if (boost::filesystem::is_directory(animationPath)) {
    animation = game->createObject<CDynamicAnimation>("CDynamicAnimation");
                 ...
                 CResourcesProvider::getInstance()->getPath(animationPath +
                                                            ".png"))) {
    animation = custom
                    ? game->createObject<CAnimation>("CCustomAnimation")
