...
void apply_tile_layer_metadata(const std::shared_ptr<CMap> &map, const json &layer) {
  ...
  if (layerProperties.count("outOfBounds")) {
    ...
    out_of_bounds_tiles.erase(level);
  }
...
void CMapLoader::loadFromTmx(const std::shared_ptr<CMap> &map,
  ...
  if (mapc) {
    ...
    for (const auto &layer : mapLayers) {
      if (vstd::string_equals(layer["type"].get<std::string>(),
                              "objectgroup")) {
...
std::shared_ptr<CMap> CMapLoader::loadNewMap(const std::shared_ptr<CGame> &game,
                                             const std::string &mapName) {
  if (std::shared_ptr<json> mapc =
          CConfigurationProvider::getConfig(getMapPath(mapName))) {
    std::shared_ptr<CMap> map =
        game->getObjectHandler()->createObject<CMap>(game);
    game->setMap(map);
...
std::shared_ptr<CMap>
CMapLoader::loadSavedMap(const std::shared_ptr<CGame> &game,
  ...
  if (std::shared_ptr<json> save = CConfigurationProvider::getConfig(path)) {
    ...
    game->getObjectHandler()->registerConfig(
        getConfigPaths(mapName)); // TODO: duplicate?
...
std::shared_ptr<CMap>
CMapLoader::loadNewMapWithPlayer(const std::shared_ptr<CGame> &game,
...
CMapLoader::createPlayer(const std::shared_ptr<CGame> &game,
                         std::string &player) {
...
std::shared_ptr<CMap>
CMapLoader::loadRandomMapWithPlayer(const std::shared_ptr<CGame> &game,
                                    std::string player) {
...
void CMapLoader::save(const std::shared_ptr<CMap> &map,
                      const std::string &name) {
  CResourcesProvider::getInstance()->save(
      vstd::join({"save/", name, ".json"}, ""), JSONIFY_STYLED(map));
}

void CMapLoader::handleTileLayer(const std::shared_ptr<CMap> &map,
...
void CMapLoader::handleObjectLayer(const std::shared_ptr<CMap> &map,
                                   const json &layer) {
  int level =
      vstd::to_int(layer["properties"]["level"].get<std::string>()).first;
  ...
  for (const auto &object : objects) {
    ...
    const json &objectProperties =
        object.count("properties") ? object["properties"] : json();
...
std::shared_ptr<CMap>
CRandomMapGenerator::loadRandomMap(const std::shared_ptr<CGame> &game) {
  ...
  for (unsigned int i = 0; i < dungeon.getCells().size(); i++) {
    for (unsigned int j = 0; j < dungeon.getCells()[0].size(); j++) {
      if (dungeon.getCells()[i][j].isOpenspace()) {
        map->addTile(map->getGame()->createObject<CTile>("GroundTile"), i, j,
                     0);
      } else {
        map->addTile(map->getGame()->createObject<CTile>("MountainTile"), i, j,
                     0);
      }
    }
  }
...
void CGameLoader::startRandomGameWithPlayer(const std::shared_ptr<CGame> &game,
                                            std::string player) {
...
void CGameLoader::changeMap(const std::shared_ptr<CGame> &game,
                            const std::string &name) {
  vstd::call_later([game, name]() {
    ...
    vstd::call_when([game]() { return !game->getMap()->isMoving(); },
                    [game, name]() {
                      ...
                      std::shared_ptr<CMap> map =
                          CMapLoader::loadNewMap(game, name);
                      game->setMap(map);
...
void CGameLoader::initConfigurations(
    const std::shared_ptr<CObjectHandler> &handler) {
  for (const std::string &path :
       CResourcesProvider::getInstance()->getFiles(CResType::CONFIG)) {
    handler->registerConfig(path);
  }
}

void CGameLoader::initObjectHandler(
    const std::shared_ptr<CObjectHandler> &handler) {
...
void CGameLoader::initScriptHandler(
    const std::shared_ptr<CScriptHandler> &handler,
    const std::shared_ptr<CGame> &game) {
  for (const std::string &script :
       CResourcesProvider::getInstance()->getFiles(CResType::PLUGIN)) {
...
void CGameLoader::loadGui(const std::shared_ptr<CGame> &game) {
  ...
  vstd::event_loop<>::instance()->registerFrameCallback(
      [game](int time) { game->getGui()->render(time); });
  vstd::event_loop<>::instance()->registerEventCallback(
      [game](SDL_Event *event) { return game->getGui()->event(event); });
}

void CPluginLoader::loadPlugin(const std::shared_ptr<CGame> &game,
