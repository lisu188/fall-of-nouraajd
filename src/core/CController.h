...
public:
  virtual std::shared_ptr<vstd::future<Coords, void>>
  control(std::shared_ptr<CCreature> c);
};
...
public:
  ...
  std::pair<bool, Coords::Direction> isOnPath(std::shared_ptr<CPlayer> player,
                                              Coords coords);
...
public:
  virtual bool control(std::shared_ptr<CCreature> me,
                       std::shared_ptr<CCreature> opponent);

  virtual void start(std::shared_ptr<CCreature> me,
                     std::shared_ptr<CCreature> opponent);
...
public:
  bool control(std::shared_ptr<CCreature> me,
...
public:
  ...
  void start(std::shared_ptr<CCreature> me,
             std::shared_ptr<CCreature> opponent) override;
...
class CTargetController : public CController {
  V_META(CTargetController, CController,
         V_PROPERTY(CTargetController, std::string, target, getTarget,
                    setTarget))
public:
  CTargetController();

  std::shared_ptr<vstd::future<Coords, void>>
  control(std::shared_ptr<CCreature> creature) override;
...
class CGroundController : public CController {
  V_META(CGroundController, CController,
         V_PROPERTY(CGroundController, std::string, tileType, getTileType,
                    setTileType))
public:
  std::shared_ptr<vstd::future<Coords, void>>
  control(std::shared_ptr<CCreature> creature) override;
...
class CRangeController : public CController {
  V_META(CRangeController, CController,
         V_PROPERTY(CRangeController, std::string, target, getTarget,
                    setTarget),
