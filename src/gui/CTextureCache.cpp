...
SDL_Texture *CTextureUtil::calculateAlpha(SDL_Renderer *renderer,
                                          SDL_Surface *surface) {
  if (!surface) {
  ...
  if (w <= 0 || h <= 0) {
    vstd::logger::error("CTextureUtil::calculateAlpha: invalid surface size", w,
                        h);
    SDL_SAFE(SDL_FreeSurface(surface));
    return nullptr;
  }

  auto secondSurface = SDL_CreateRGBSurfaceWithFormat(
      surface->flags, surface->w, surface->h, 32, SDL_PIXELFORMAT_RGBA32);
  ...
  std::unordered_set<std::pair<int, int>> mask =
      shouldAddMask ? calculateMask(surface)
  ...
  auto texture =
      SDL_SAFE(SDL_CreateTextureFromSurface(renderer, secondSurface));
...
std::unordered_set<std::pair<int, int>>
CTextureUtil::calculateMask(SDL_Surface *pSurface) {
