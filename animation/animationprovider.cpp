...
Animation *AnimationProvider::getAnim(std::string path, int size) {
	...
	if (instances.find(size) == instances.end()) {
		instances.insert(
				std::pair<int, AnimationProvider *>(size,
						new AnimationProvider(size)));
	}
	return instances.at(size)->getAnimation(path);
}

void AnimationProvider::terminate() {
	for (std::map<int, AnimationProvider *>::iterator it = instances.begin();
			it != instances.end(); it++) {
		delete (*it).second;
	}
	instances.clear();
...
AnimationProvider::AnimationProvider(int size)
		: tileSize(size) {
}
...
void AnimationProvider::loadAnim(std::string path) {
	...
	for (int i = 0; true; i++) {
		...
		if (!image.hasAlphaChannel()
				&& path.find("tiles") == std::string::npos) {
			image.setMask(image.createHeuristicMask());
		}
		if (!image.isNull()) {
			img = new QPixmap(
					image.scaled(tileSize, tileSize, Qt::IgnoreAspectRatio,
