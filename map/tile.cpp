...
Tile::Tile(std::string name, int x, int y, int z)
	...
	setXYZ(x, y, z);
	Json::Value config =
			(*ConfigurationProvider::getConfig("config/tiles.json"))[className];
	loadFromJson(config);
}
...
Tile::Tile(const Tile &tile)
		: Tile(tile.className, tile.getPosX(), tile.getPosY(), tile.getPosZ()) {
}
...
void Tile::moveTo(int x, int y, int z, bool silent) {
	if (init && map) {
		...
		setXYZ(x, y, z);
		map->insert(
				std::pair<Coords, std::string>(Coords(posx, posy, posz),
						className));
	}
