...
void FightView::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
		QWidget *widget) {
...
void CreatureFightView::paint(QPainter *painter,
	...
	item->setPos(((boundingRect().width() - item->boundingRect().width()) / 2),
			0);
	painter->fillRect(0, boundingRect().height() - 50, boundingRect().width(),
			25, QColor("ORANGE"));
	...
	painter->fillRect(0, boundingRect().height() - 25, boundingRect().width(),
			25, QColor("CYAN"));
	painter->fillRect(0, boundingRect().height() - 25,
			boundingRect().width() * creature->getManaRatio() / 100.0, 25,
			QColor("BLUE"));
